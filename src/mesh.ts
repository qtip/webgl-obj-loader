...
function* triangulate(elements: string[], verts: number[]) {
    if (elements.length <= 3) {
        ...
        const indices: Triangulate.PolygonIndex[] = elements.map((e) => +e.split('/')[0] - 1);
        const polygon: Triangulate.Polygon3 = indices.map((i) => [verts[i * 3], verts[i * 3 + 1], verts[i * 3 + 2]]);
        for (let triangle of Triangulate.triangulate(polygon)) {
            yield [
                elements[triangle[0]],
                elements[triangle[1]],
                elements[triangle[2]]
